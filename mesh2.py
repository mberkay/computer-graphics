...
class Mesh2:
    def __init__(self, positions: List[Point], faces, face_type=FaceType.QUAD):
        self.positions = positions
        # if faces is None:
        #     faces = Mesh2.create_faces(positions)
        self.faces = faces
    ...
    def subdivide(self, type):
        ...
        if type is 1 and self.subdivision_level < 4:
            ...
            if len(self.subdivisions) == next_subdivision_level:
                can_subdivide, subdivided = self.create_subdivided_mesh(deepcopy(self.positions), deepcopy(self.faces), self.face_type)
                if can_subdivide:
    ...
    def create_subdivided_mesh(self, positions, faces, face_type):
        if face_type == FaceType.QUAD:
            ...
            for face in faces_to_append:
                faces.append(face)
            return True, Mesh2(positions, faces, face_type)
        return False, None
